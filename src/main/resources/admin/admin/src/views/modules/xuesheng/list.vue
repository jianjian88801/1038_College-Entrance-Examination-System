<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','入学办理') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="ruxuebanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'入学办理':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','入学办理') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="ruxuebanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'入学办理':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','入学办理') && contents.tableBtnIcon == 0" type="success" size="mini" @click="ruxuebanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'入学办理':'' }}</el-button>
    ...
    <ruxuebanli-cross-add-or-update v-if="ruxuebanliCrossAddOrUpdateFlag" :parent="this" ref="ruxuebanliCrossaddOrUpdate"></ruxuebanli-cross-add-or-update>
...
import ruxuebanliCrossAddOrUpdate from "../ruxuebanli/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      ruxuebanliCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    ruxuebanliCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.ruxuebanliCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$nextTick(() => {
      this.$refs.ruxuebanliCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
